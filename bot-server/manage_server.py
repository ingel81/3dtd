"""
Bot-Server lifecycle manager.

Start/stop/restart/status of the WebSocket bot server as a background process.
Tracks PID in `.server.pid`, routes stdout/stderr to `.server.log`.
...
def _python_executable() -> str:
    ...
    Important: the server needs the packages installed in bot-server/venv
    (websockets, fastapi, uvicorn). If manage_server was launched via a
    system python, sys.executable would NOT have those packages.
    """
    if os.name == "nt":
