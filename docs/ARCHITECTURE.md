...
| `terrain-queries.ts` | Raycasts gegen Boden und Tiles: Säulen-Probe `sampleColumn()` mit Cache pro 0,5-m-Säule (Strahl in deren Mitte, `columnCentre`) und `lodVersion`, `getGroundHeightEstimate()`, Tile-LOD-Peek ohne Raycast, Straßen-Freiraum für den Routen-Korridor (`measureStreetClearance()`, Säulen am genauen Punkt am Cache vorbei), Line-of-Sight. Als `engine.terrain` erreichbar, nur `getTerrainHeightAtGeo()` reicht der Engine durch |
| `scene-environment.ts` | Statische Szenen-Lichter (`addSceneLights()`) und der Himmel als Cube-Textur aus `day.webp` (`SkyBackground`, nach einem WebGL-Context-Restore neu umgerechnet) |
...
| **TowerPlinthRenderer** | `renderers/tower-plinth/` | Steinsockel unter Towern auf unebenem Grund (`engine.plinths`), ein Mesh pro Sockel, Bruchsteinmauerwerk prozedural im `MeshStandardMaterial` (`onBeforeCompile`). Angelegt und entfernt vom `TowerManager`, Höhe aus `Tower.plinthHeight`; wo er über einen Abbruch ragt (`Tower.plinthOverhang`), gestufte Kragsteine in derselben Geometrie (`plinth-braces.ts`), siehe [TOWER_CREATION.md → Sockel auf unebenem Grund](TOWER_CREATION.md#sockel-auf-unebenem-grund) |
| **TowerBadgeRenderer** | `renderers/tower-badge/` | Veteranen-Abzeichen über Towern mit Rang und das Pause-Zeichen über einem Tower mit Feuerpause (`engine.towerBadges`), alle in einem Draw Call, Billboard und Insignien im Shader. Rang aus `CombatComponent.kills`, jeden Frame vom `TowerManager` gesetzt (`syncVeteranBadges`), die Feuerpause von `TowerLifecycle.setHoldFire`, siehe [TOWER_CREATION.md → Veteranen-Ränge](TOWER_CREATION.md#veteranen-ränge) |
