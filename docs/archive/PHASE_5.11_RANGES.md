...
> | Range-Interpolation + DPS-skalierte Caps (5.11b) | `wave-config-builder.ts::buildWaveConfig` |
...
- `src/app/ai/core/templates.ts`: damals 19 Templates mit Ranges, heute 22 (Backend liest sie über das generierte Schema)
- `training-backend/config.py`: NUM_CONTINUOUS=4, MAX_WAVE_DURATION_MS, Damage-Thresholds
- `training-backend/core/model.py`: params_head (4,), factors in [0,1] via sigmoid
- `training-backend/server.py::_decode_action`: lerp + Duration-Cap
- `training-backend/core/reward.py::_drama_reward`: narrower sweet, zero-damage penalty
- `src/app/ai/core/templates.ts`: 1:1 Mirror
- `src/app/ai/core/onnx-policy.ts::decodeModelOutput`: heute nur Template und Faktoren; Lerp und Duration-Cap liegen in `wave-config-builder.ts`
...
> Der mod-30-Loop gilt nur noch für den statischen Wave-Fallback; das
> Gold-Budget verfällt ab W31 (siehe unten).

**Endgame HP-Multiplier** (`configs/wave-curriculum.config.ts::endgameHpMultiplier`):
- W1-20: ×1.0
- ab W21: +5 %/Wave
- Cap: ×4.0 bei W80
- Wirkt **post-NN**, compoundet auf NN's `hp_mult`

**Per-Leak Damage-Skalierung** (`configs/wave-curriculum.config.ts::enemyBaseDamageForWave`):
- W1-10: 1 HP pro Leak
- W11-20: 2 HP
- W21-30: 3 HP
...
> Boss-Wellen nach W30 zahlen das Doppelte (`BOSS_GOLD_MULTIPLIER = 2`, seit
> 2026-09-11).
...
- Nur die Template-Sequenz spiegelt sich ins Backend (heute für die Maske; den Decoder-Override gibt es nicht mehr)
...
> **Stand 2026-09:** Der Encoder ging über Schema v2 (162), v3 (203) und v4 (207)
> auf **v5 (208 Features)**; alte Checkpoints sind nicht ladbar. Wichtiger: Es wird kein Modell
