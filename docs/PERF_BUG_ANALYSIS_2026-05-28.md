...
- **R9 Befund, der Kern des Findings gilt nicht mehr.** three r186 rechnet ein Equirect-`scene.background` einmal in ein Cube-Render-Target um (`WebGLEnvironments.getCube()`, Kantenlänge gleich Bildhöhe) und zeichnet eine Box mit Cube-Lookup, `atan/asin` pro Pixel gibt es nicht. Übrig ist Speicher: `day.webp` ist 4096×2048, das Cube-RT hat 6 × 2048² RGBA8 mit Mips (etwa 134 MB), und die Equirect-Quelle bleibt daneben im VRAM (etwa 45 MB mit Mips), obwohl sie nach der Umrechnung niemand mehr liest. Vorschlag: in `setupSky()` selbst `new WebGLCubeRenderTarget(image.height).fromEquirectangularTexture(renderer, texture)` aufrufen, die RT-Textur als Background setzen, die Quelle disposen und das RT in `dispose()` freigeben. Das ist laut three-Quelltext derselbe Pfad wie intern, also pixelgleich. Nicht umgesetzt, weil es `setupSky()` und `dispose()` in `three-tiles-engine.ts` anfasst, das gerade parallel zerlegt wird, und der GPU-Pfad ohne Browser nicht prüfbar ist. Eine 1024er-Cube spart weitere 100 MB, macht den Himmel aber weicher, also nur mit Playtest.
