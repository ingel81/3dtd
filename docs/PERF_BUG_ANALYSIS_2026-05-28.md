...
- **Raycasts gegen die Tiles messen (BVH-Entscheidung).** `instrumentRaycasts()` (`utils/raycast-stats.ts`) umhüllt `tilesRenderer.group.raycast` einmal. Damit ist jeder Ray in die Tiles erfasst, auch die der Kamera-Steuerung. Aufrufer benennen sich mit `raycastStats.enter()`/`exit()`, der äußerste gewinnt: `routeGrid` (Zell-Sampling und Sweeps der Route-Grid), `intro`, `streets`, `towerRange` (Reichweiten-Scheibe beim Platzieren und Auswählen), `lineOfSight`, `screenPick` (Zeiger im Bau- und Platziermodus, Klicks), `tileProbe` (Origin-Probe nach jedem Tile-Load), `heightAtGeo` (alles andere über `getTerrainHeightAtGeo`), `heightAtLocal`, `cameraControls` (die GlobeControls: Punkt unter der Kamera pro Frame, Zoom-Punkt, Pivot; seit 2026-09-13 über `GroundPickRoot` nur gegen die Tiles, seit 2026-09-14 mit Raycast-Cache: ein wiederholter Strahl bei unverändertem Tile-Set kommt aus dem Speicher und taucht nicht auf, in Ruhe steht die Zeile also fast still). Ohne Aufrufer landet ein Ray unter `unscoped`. Treffer im Spalten-Cache kosten keinen Ray und tauchen nicht auf. In der Konsole gibt `__raycastStats()` pro Aufrufer Aufrufe, Summe, Mittel, Maximum, den längsten Burst (Rays mit weniger als 4 ms Abstand, also eine Schleife am Stück) und Treffer pro Ray aus, `__raycastStats(true)` setzt danach zurück. Kosten ohne Abfrage: zwei `performance.now()` und ein Map-Zugriff pro Ray.
