...
Weitere Keys liegen in Services, nicht in Stores: `td_favorites_v2` und `td_recent_locations_v1` (`LocationManagementService`), `td_onboarding_v1` (`OnboardingService`), `td_debug_windows_v6` (`DebugWindowService`), `td_music_enabled` (`BackgroundMusicService`), `training-timescale` (`GameStateManager`), `td_geocode_cache_v1` (`GeocodingService`), `3dtd-tile-credentials` (`ConfigService`).
