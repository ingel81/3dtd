...
| tank | heavy | 250 | 3 | – | Mechanisch, `canBleed: false`. Quaternius-Modell (2026-09-15) in Metern, die Ketten laufen (`TankArmature\|Tank_Forward`, `animationSpeed` passend zu 3 m/s); Tarnanstrich mit Kantenabrieb und Staub als gebackene Textur (`weathered_paint`) |
...
der Todesanimation zählen dort mit (`getKillingCount()`). Die 5 % sind gewählt, nicht am
Bildschirm abgeglichen. Knapp ist das Mammut: Es ruht bei etwa 1,9 s und ist bei 2,0 s weg (2,8 %
Bewegung); auch die drei Typen mit eigener Dauer liegen danach nur kurz still.
...
- **Welle:** Die Welle wartet auf die Kinder, sie leben ja. Das Kill-Gold
  verteilt sich nach Gewicht auf alle Körper (`WaveManager.getExpectedBodyWeight()`), ein Split
...
- **Grenzen:** Kopf- und Schwanzwechsel beim Zerfall sind ein harter Poolwechsel; das
  Schwanzstück (reicht 5,5 m hinter seinen Ring) erscheint schlagartig. Alle Ringe tragen
  dieselbe Textur, Normal Maps kann der VAT-Shader nicht. In der Ecke springt der Abstand der
  Ringe am Anfang des Bogens (der Ort nicht). Eine Ideallinie (außen anfahren, innen am
  Scheitel) gibt es nicht; sie hätte auf schmalen Straßen etwa doppelt so viel Radius.
...
| Zielen | Jeder Tower zielt auf den nächsten Punkt des Körpers in Reichweite und Sicht. Je Tower und Pfad sind die Punkte einmal sortiert (Station, quer zum Tower verschoben bis zur seitlichen Grenze, damit der Punkt in einer Route-Zelle liegt); je Tower-Zug gilt der nächste Punkt zwischen Schwanz und Spitze, dessen Zelle der Tower am Boden sieht; ein Punkt ohne Antwort der Zelle gilt als nicht gesehen, kein Raycast. Die Punktliste wird neu gebaut, wenn sich Reichweite oder Grid-Generation ändern | `BodyAim` (`services/combat/body-aim.ts`), `Tower.findTarget(…, bodyDistSq)` |
| Randzellen | Seit 2026-09-24 hat ein Tower Antworten für jede Zelle, deren Fläche die Reichweite anschneidet, nicht nur für die mit Mittelpunkt darin ([LOS_PIPELINE.md](LOS_PIPELINE.md#reichweite-in-zellen)). Der Ring bis etwa 1,4 m, in dem BodyAim vorher per Raycast traf und ein Klumpen kein Kandidat war, ist damit geschlossen | `GlobalRouteGrid.forEachSlotInReach`, `BodyAim.resolve` |
| Diagnose | `__towerTargets()` in DevTools: je Tower mit einem Gegner in der Nähe eine Zeile, warum er ein Ziel hat oder keins (kein Kandidat in `visibleCells` samt den Antworten der Zellen, Kandidaten nur außerhalb der Reichweite, noch nicht genommen, LOS noch offen; ein schlafender Tower bekommt dieselbe Auswertung hinter `asleep, `; am Ende, wie viele seiner `visibleCells` nicht mehr im Grid stehen, etwa nach einem Neuaufbau des Korridors; mit Ziel Cooldown und Turm-Ausrichtung). `__towerTargets.watch()` loggt nach dem Zerfall einer Ooze 6 s lang (oder bis alle Klumpen weg sind) jede Sekunde eine Zeile je Tower nahe den Klumpen; zerfällt in der Zeit eine zweite Ooze, folgt die Sonde nur noch deren Klumpen. `__towerTargets.watch(false)` beendet das | `TowerTargetConsole` (`services/debug/tower-target-console.ts`) |
| Treffpunkt | Projektile fliegen zum Zielpunkt (`Projectile.aimPoint`), auch wenn die Ooze unterwegs stirbt; Flammenkegel, Tentakel und Blitz treffen dort. Zeigt der Flammenkegel auf ein anderes Ziel und liegt der Zielpunkt daneben, prüft er noch die Körperpunkte nächst seinem Ende und seiner Mitte (`bodyPointInCone`, `utils/body-cone.ts`), ohne eigene Sichtprüfung für diesen Punkt; kreuzt der Kegel den Körper schräg zwischen diesen beiden Stichproben, kann ein Treffer fehlen. Blut, Schadenszahlen, Eis und DoT-Zahlen erscheinen am Punkt des letzten Treffers; ein DoT-Tick zieht ihn vorher auf den nächsten Punkt des Körpers nach, falls der Schwanz schon daran vorbei ist | `RouteBody.hit`, `enemyHitSpot` (`utils/enemy-hit-spot.ts`), `CombatEffectService.keepHitOnBody` |
