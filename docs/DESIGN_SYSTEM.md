...
- Screenshot: `captureFrame()` kopiert den nächsten gezeichneten Frame synchron in `RenderLoop.onNextFrameRendered()`, weil der Renderer ohne `preserveDrawingBuffer` läuft. Logos und Attribution werden ins Bild gestempelt (auf dem Schirm sind sie HTML), dazu das Zeichen des Spiels (`stampScreenshot` in `utils/screenshot.ts`): unten links hinter den Anbieter-Logos dezent `https://3dtd.sgeht.net` (Schriftgröße der Attribution, 500 `--td-font-body`, `--td-text-primary` mit 75 % Deckkraft und weichem Schatten); reicht die Attributionsleiste so weit nach links, dass die Adresse darunter läge (schmales oder hochformatiges Bild, lange Attribution), steht sie eine Zeile höher über den Logos am linken Rand. Unten rechts über der Attribution das Logo (`assets/images/logo/logo.png`) als Wasserzeichen, viermal so hoch wie die Schrift (etwa 48 px bei 1080p) mit 60 % Deckkraft. Beides auch in der DevWorld, wo keine Anbieter-Logos stehen. Dann PNG-Download als `3dtd-<ort>-<datum>-<zeit>.png`
