...
Beim Befehl `command:start-wave` nimmt der `GameStateManager` einen Snapshot (`captureSnapshot`), bevor die Welle
etwas ändert: Uhr, Zufallsstand, Id-Zähler, Credits, HQ-Leben, Wellennummer, Forschung, Fähigkeiten, Held, Tower
(Upgrades, Zustand, Drehung, Sicht-Maske), bemannter Tower, Warteschlange der Luft-Nachrüstung. Das sind wenige KB.
...
Schlag einer Fähigkeit. Sonst bleibt die Welle ohne Snapshot und ist nicht abspielbar. Cheats, die am Befehlslog
vorbei Gegner setzen, entfernen oder töten (`debug:spawn-enemy`, `debug:remove-enemy`, `debug:kill-all`), markieren
die laufende Welle ebenso.
...
- **Verlassen:** Den Live-Snapshot laden, solange der Replay-Modus noch an ist, dann aus. Die Stores haben während
  des ganzen Replays nichts gehört und stehen danach auf dem Live-Stand.
...
- Beim Springen die Sounds und Effekte der übersprungenen Strecke; was bei der Ankunft auf dem Feld steht, zeigt es
...
| Spec | Prüft |
|------|-------|
| `integration/resimulation.scenario.spec.ts` | Abnahme: Welle mit Befehlen mittendrin bit-genau nachgerechnet; Zurückspringen; Live-Stand zurück; Ooze, Wurm, Skelette; Datei in einem frischen Spiel; fremde Welt und Balance abgelehnt |
...
| `managers/hero.manager.spec.ts`, `ability.manager.spec.ts`, `entities/tower.entity.spec.ts` | Sichern und Laden je Teil |
