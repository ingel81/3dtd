...
| `enemy:reached-base` | EnemyManager, bei einer Ooze `OozeBodies` | GameStateManager (Schaden, gedeckelt durch `maxLeakDamagePerWave`), WaveManager, GameStateSyncService, AIDataCollector, LeakVignetteComponent (roter Rand, gedrosselt), RunStatsTracker | Enemy am Ziel (`enemy`, `damage`) |
...
| `tower:placed` | TowerManager | GameStateSyncService, DpsBinsOverlay (DPS-Anzeige der VisualizationFacade, nur solange sie an ist), AIDataCollector, RunStatsTracker, OnboardingService | Tower gebaut (`tower`, `position`, `cost`). Die Kosten zieht `GameStateManager.placeTower()` (`TowerLifecycle.place()`) direkt ab |
...
| `tower:sold` | TowerManager | GameStateSyncService, DpsBinsOverlay, LosDebugService, AIDataCollector, RunStatsTracker | Tower verkauft (`tower`, `refund`). Die Gutschrift macht `GameStateManager.sellTower()` (`TowerLifecycle.sell()`) direkt |
...
| `ability:rejected` | AbilityManager (`use()`) | RefusalHintService (Hinweis, nur bei einem Einsatz des Spielers) | Einsatz abgelehnt (`abilityId`, `reason`: `locked`, `no-charge`, `no-wave`, `no-route`, `unknown`) |
| `ability:state-changed` | AbilityManager | GameStateSyncService, OnboardingService | **Snapshot-Event** nach Freischaltung, Einsatz, Einschlag und Nachladen (`abilities`). Füllt `GameStore.abilities` |
...
| Event | Producer | Consumer | Beschreibung |
|-------|----------|----------|--------------|
| `command:place-tower` | TowerPlacementService (der Trainings-Bot ruft `GameStateManager.placeTower()` direkt) | GameCommandsHandler → GameStateManager.placeTower() | Tower platzieren (`position`, `typeId`, `rotation?`, `plinthHeight?` für den Sockel) |
...
| **VFXService** | Nein | Subscriber | Reagiert auf `vfx:*`, `enemy:split`, `ability:used`, `ability:impact`, `hero:level-up`, `game:reset` |
| **AudioService** | Nein | Subscriber | Reagiert auf `audio:play`, `ability:impact`, `game:reset` |
| **ScreenShakeService** | Nein | Subscriber | Reagiert auf `vfx:projectile-impact`, `health:changed`, `ability:impact`, `enemy:died` (Boss) |
