...
| `tower:manual-shot` | GameStateManager (nach `updateMannedTower`, deferred) | TowerControlService (Rückstoß im HUD) | Der besetzte Tower hat gefeuert (`towerId`), auf `target` oder daneben (`null`, Fehlschuss) |
...
| `command:tower-trigger` | TowerControlService (linke Maustaste) | GameCommandsHandler → GameStateManager.setMannedTrigger() | Abzug des besetzten Towers gedrückt oder losgelassen (`held`). Die Zielrichtung ist kein Command (`GameStateManager.setMannedAim`, pro Mausbewegung) |
