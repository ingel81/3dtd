...
describe('BodyAim', () => {
  ...
  beforeEach(() => {
    ...
    aim.beginTower(tower, TOWER_X, TOWER_Z);
  });
  ...
  describe('without a cell answer', () => {
    /** Cells without an answer of the tower (off its reach, not resolved) */
    ...
    it('counts a point without an answer of the tower as not seen, as a single enemy there', () => {
      cellsWithoutAnswers();
      expect(aim.distSq(oozeBetween(0, 200))).toBe(Infinity);
      expect(aim.aim(oozeBetween(0, 200), point())).toBe(false);
    });

    it('counts a point off the grid as not seen', () => {
      grid.getCellAt = () => undefined;
      generation++;
      expect(aim.distSq(oozeBetween(0, 200))).toBe(Infinity);
    });

    it('takes the answers once the LOS of the tower is resolved in the same cells (registerTower, a mask)', () => {
      cellsWithoutAnswers();
      const ooze = oozeBetween(0, 200);
