...
export interface ScheduleBuildConfig {
  ...
  /** The run's spawn stream (GameRng); `Math.random` when none is given. */
  random?: () => number;
}
...
export function buildSpawnSchedule(config: ScheduleBuildConfig): SpawnSchedule {
  ...
  switch (config.pattern) {
    ...
    case 'clustered':
      ...
      break;
    case 'random':
      entries = buildRandom(validGroups, random);
      break;
    case 'front-loaded':
...
function buildRandom(groups: WaveEnemyGroup[], random: () => number): SpawnEntry[] {
