 ...
 * anywhere in the project, so `leakMultiplier` stayed at 1.0 for the lifetime
...
describe('gate wiring', () => {
  ...
  describe('completed waves reach the gate', () => {
    it('subscribes to the collector on construction', () => {
      ...
      for (let i = 0; i < LEAK_ADAPT_WINDOW * 3; i++) {
        collector.emitWaveResult(waveResult([0.2, 0.4, 0.3]));  // nothing arrived
      }
      expect(director.leak.leakMultiplier).toBeGreaterThan(1);
    });
    ...
    it('backs off when a wave ends the run', () => {
      ...
      const opened = director.leak.leakMultiplier;
      expect(opened).toBeGreaterThan(1);

      collector.emitWaveResult(waveResult([1, 1, 1], false));
      expect(director.leak.leakMultiplier).toBeLessThan(opened);
    });
  ...
  describe('the gate changes the wave that ships', () => {
    it('a wider budget ships bigger waves', async () => {
      ...
      // `leak.leakMultiplier` argument in buildWaveConfig must fail this.
      //
      ...
      expect(director.leak.leakMultiplier).toBeGreaterThan(2);
