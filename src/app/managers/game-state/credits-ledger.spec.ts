...
describe('CreditsLedger', () => {
  it('books a delta and announces the new total', () => {
    ...
    expect(changed).toHaveBeenCalledWith({ type: 'credits:changed', credits: START + 25, delta: 25, source: 'kill' });
  });
  ...
  it('spends only what is there, and books nothing otherwise', () => {
    ...
    expect(ledger.spend(START + 1, 'build')).toBe(false);
    expect(changed).not.toHaveBeenCalled();
    expect(ledger.spend(START, 'build')).toBe(true);
  ...
  it('resets to the start credits as one delta', () => {
    ...
    expect(changed).toHaveBeenCalledWith({ type: 'credits:changed', credits: START, delta: 40, source: 'reset' });
  });
});
