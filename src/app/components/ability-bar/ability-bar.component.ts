...
import { AbilityBarHero, abilityButtonView, abilityTooltip, heroTooltip } from './ability-button';

/**
 * Bar at the left edge of the playfield: at the top the hero's button, when
 * there is a hero, under it one button per ability in ABILITIES order. Every
 * ability shows, locked ones dimmed until their research is done; a press
 * arms or leaves the targeting mode like the ability's key.
 *
  ...
  readonly buttons = computed(() => {
    ...
    return ABILITY_IDS.map((id) => {
      ...
      return {
        ...
        view,
        tooltip: abilityTooltip(config, status, view, getResearch(config.researchId)),
      };
    });
  });
