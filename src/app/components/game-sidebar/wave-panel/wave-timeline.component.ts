 ...
 * number under each, small icons above for boss and air. Under the line one
 * detail line for one mark (shownPeek): the next wave, the one under the
 * pointer or keyboard focus, or the one clicked last.
 */
@Component({
...
export class WaveTimelineComponent {
  ...
  /** "Wave 7, Bat Swarm, air" */
  markLabel(peek: WavePeek): string {
    return [`Wave ${peek.wave}`, peek.name, ...(peek.air ? ['air'] : [])].join(', ');
  }
}
