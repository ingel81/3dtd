...
import { buildWaveConfig, type GateReading } from './wave-config-builder';
...
const gate = (budgetMultiplier = 1): GateReading => ({ budgetMultiplier, status: new GateController().status });

function decision(templateIdx: number, factors: Partial<DirectorDecision> = {}): DirectorDecision {
  return {
    templateIdx,
    countFactor: 0.5,
    spawnFactor: 0.5,
    hpFactor: 0.5,
    variationFactor: 0.5,
    why: { by: 'model', candidates: 1, probability: 0.7 },
    ...factors,
  };
}
...
const build = (d: DirectorDecision, state: GameStateSnapshot, g = gate()) =>
  buildWaveConfig(d, state, buildWaveContext(state).maskReason, g);
...
describe('buildWaveConfig', () => {
  ...
  it('reports the model probability as confidence, and 1 for a rule decision', () => {
    expect(build(decision(0), defense(1e6)).confidence).toBe(0.7);

    const rules = decision(0, {
      why: { by: 'rules', candidates: 3, lastRanWavesAgo: null, history: 0, tied: 1, ramp: 0.5 },
    });
    expect(build(rules, defense(1e6)).confidence).toBe(1);
  });
  ...
  it('narrows the count range for a weak defense', () => {
    const strong = build(decision(0, { countFactor: 1 }), defense(1e6, 1e6));
    const weak = build(decision(0, { countFactor: 1 }), defense(1e6, 0));
  ...
  it('ships a bigger wave for a wider gate budget where the cap binds', () => {
    ...
    const tight = build(decision(0, { countFactor: 1 }), state, gate(1));
    const wide = build(decision(0, { countFactor: 1 }), state, gate(4));
  ...
  it('compresses the spawn delay of a wave that would run past three minutes', () => {
    const config = build(decision(0, { countFactor: 1, spawnFactor: 1 }), defense(1e6));
