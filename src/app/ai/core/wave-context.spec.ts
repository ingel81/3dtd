...
describe('buildWaveContext() mask reason', () => {
  ...
  function reasonsFor(state: GameStateSnapshot): string[] {
    const { maskReason } = buildWaveContext(state);
    return explainWaveDecision({
      wave: state.waveNumber + 1,
      templateName: 'Any',
      mask: maskReason,
      director: { by: 'rules', candidates: 3, lastRanWavesAgo: null, history: 0, tied: 3, ramp: 0.7 },
      gate: { multiplier: 1, samples: 0, meanLeak: null, lastStep: 'warming-up' },
      sizing: {
  ...
  it('holds nothing back when only chaos towers stand, normal and boss wave', () => {
    for (const wave of [40, 44]) {                 // planning 41 (free) and 45 (boss)
      ...
      expect(buildWaveContext(state).maskReason.heldBack, `wave ${wave + 1}`)
  ...
  it('still holds air and ethereal back for a cannon-only defense', () => {
    ...
    const { heldBack } = buildWaveContext(state).maskReason;
