...
describe('WaveOutcomeTracker', () => {
  ...
  it('counts spawns, kills and leaks per wave and per type', () => {
    ...
    const outcome = tracker.finalize('completed', 300, 1);
  ...
  it('ends a lifetime at death or base arrival and counts the rest up to the end', () => {
    ...
    const outcome = tracker.finalize('completed', 4000, 1);  // c: 2000
  ...
  it('divides the time metrics by the timescale', () => {
    tracker.start(1, 100, 1000);
    tracker.enemySpawned('a', 'zombie', 1000);
    tracker.enemyDied('a', 'zombie', 0.2, 1800);

    const outcome = tracker.finalize('completed', 2000, 4);

    expect(outcome.waveDurationMs).toBe(250);
    expect(outcome.avgEnemyLifetimeMs).toBe(200);
    expect(outcome.enemyPerformance['zombie'].avgLifetimeMs).toBe(200);
  });
  ...
  it('marks a wave that destroyed the base as lost and a close call at zero health', () => {
    ...
    const outcome = tracker.finalize('base-destroyed', 2000, 1);
  ...
  it('reports an empty wave with zeroed progress and lifetime', () => {
    tracker.start(0, 100, 0);
    const outcome = tracker.finalize('completed', 500, 1);
  ...
  it('forgets the running wave on reset, keeping the clock', () => {
    ...
    const outcome = tracker.finalize('completed', 3000, 1);
  ...
  it('counts split children as spawned bodies', () => {
    ...
    expect(tracker.finalize('completed', 0, 1).enemiesSpawned).toBe(3);
  });
  ...
  it('adds up ability kills from zero at every wave start', () => {
    ...
    expect(tracker.finalize('completed', 0, 1).abilityKills).toBe(3);

    tracker.start(3, 100, 0);
    expect(tracker.finalize('completed', 0, 1).abilityKills).toBe(0);
  });
});
