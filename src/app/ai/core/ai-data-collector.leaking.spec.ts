 ...
 */
describe('AIDataCollectorService oozes flowing in', () => {
  ...
  beforeEach(() => {
    ...
    injections['GameStateManager'] = { getEventBus: () => bus, trainingTimescale: () => 1 };
    injections['TowerDefenseStore'] = { baseHealth: () => 100 };
    const collector = new AIDataCollectorService();
    results = [];
  ...
  it('books an ooze killed halfway in as a leak, not a kill', () => {
    ...
    bus.emit({ type: 'enemy:leaking', enemy, damage: 1 });
    bus.emit({ type: 'enemy:died', enemy, credits: 0 });
    completeWave(45);
