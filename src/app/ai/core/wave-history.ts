/**
 * The last completed waves and the per-wave series the AI reads from them
 * (damage, progress, near misses, enemy types, threat), trimmed together.
 */
...
export class WaveHistory {
  ...
  add(result: WaveResult): void {
    ...
    this.threat.push(calculateWaveThreat(result.config));
  ...
  summary(): RecentHistory {
    ...
    const lastWaveThreat = this.threat.length > 0
      ? this.threat[this.threat.length - 1]
      : 0;
