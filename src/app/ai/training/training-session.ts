 ...
 * Training Session
 *
 * WebSocket-Client zum lokalen Python-Trainings-Backend plus StrategyBot:
 * Lebenszyklus, Entscheidungs-Tick und Ausführung der Bot-Actions.
 *
 * Wird ausschließlich per dynamischem Import aus dem TrainingClientService
...
/**
 * Muss in einem Injection-Context entstehen (`store`, `dataCollector` per
 * `inject()`); der TrainingClientService legt sie über `runInInjectionContext`
...
export class TrainingSession {
  private readonly dataCollector = inject(AIDataCollectorService);
  ...
  constructor(private readonly signals: TrainingSignals, deps: TrainingDeps) {
  ...
  private handleMessage(msg: ServerMessage): void {
    switch (msg.type) {
      ...
      case 'stats':
        this.signals.stats.set(msg.data);
        break;
