...
const station = (over: Partial<BandStation> = {}): BandStation => ({
  ...
  kind: 'band', backbone: { offset: 0, y: 0 }, left: -3, right: 3, centre: 0,
  ...over,
});
...
describe('portalGround', () => {
  ...
  it('gives a tunnel portal the backbone where its column came down on a roof over the street', () => {
    ...
    // Within roofRise it keeps its hit, and so does a portal without a band.
    expect(portalGround(1, 1, 2, ground(flat))).toBeNull();
    expect(portalGround(1, 1, 5, ground(flat, null))).toBeNull();
    expect(portalGround(1, 1, 5, ground(flat, station({ kind: 'passage', backbone: null })))).toBeNull();
  });
