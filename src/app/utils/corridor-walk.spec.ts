...
import { WalkCapSegment, cellWalkable, walkCaps } from './corridor-walk';
...
describe('cellWalkable', () => {
  ...
  it('tells only for an edge cell on the ground with a fine sample of its own', () => {
    expect(cellWalkable(cell(), flat, 2)).toBe(false);
    expect(cellWalkable(cell({ terrainHeight: 0.2 }), flat, 2)).toBe(true);
    // The centre line runs through it, a deck, a filled or coarse sample, no column on the centre line, a seam.
    expect(cellWalkable(cell({ axisZ: 5 }), flat, 2)).toBeNull();
    expect(cellWalkable(cell({ surface: 'deck' }), flat, 2)).toBeNull();
    expect(cellWalkable(cell({ sample: { state: 'filled', sampledAt: 0, tileDepth: 0, tileGeometricError: Infinity } }), flat, 2))
      .toBeNull();
    expect(cellWalkable(cell({ sample: { state: 'stable', sampledAt: 1, tileDepth: 12, tileGeometricError: 20 } }), flat, 2))
      .toBeNull();
    expect(cellWalkable(cell(), () => null, 2)).toBeNull();
    expect(cellWalkable(cell({ terrainHeight: 60 }), flat, 2)).toBeNull();
  });
});
